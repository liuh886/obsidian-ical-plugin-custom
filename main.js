...
function createTaskFromLine(line, fileUri, dateOverride, body = "") {
  ...
  return new Task(taskStatus, taskDates, summary, fileUri, body);
}
...
var TaskFinder = class {
  ...
  async findTasks(file, listItemsCache, headings) {
    ...
    for (let i = 0; i < taskPositions.length; i++) {
      ...
      const task = createTaskFromLine(markdownLine, fileUri, dateOverride, body);
      if (task) {
        results.push(task);
      }
    }
  ...
  async display() {
    ...
    new import_obsidian6.Setting(containerEl).setName("Target directory").setDesc('Specify where we should look for tasks. Choose "/" to look in the whole vault.').addDropdown(
      (dropdown) => {
        ...
        dropdown.setValue(settings.rootPath);
